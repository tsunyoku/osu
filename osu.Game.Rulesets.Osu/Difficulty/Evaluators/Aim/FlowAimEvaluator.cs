...
namespace osu.Game.Rulesets.Osu.Difficulty.Evaluators.Aim
{
    public static class FlowAimEvaluator
    {
        ...
        public static double EvaluateDifficultyOf(OsuDifficultyHitObject currObj, bool withSliderTravelDistance)
        {
            ...
            var lastLastObj = (OsuDifficultyHitObject)currObj.Previous(1);
            ...
            flowDifficulty *= 1 + Math.Min(0.25,
                Math.Pow((Math.Max(currObj.AdjustedDeltaTime, lastObj.AdjustedDeltaTime) - Math.Min(currObj.AdjustedDeltaTime, lastObj.AdjustedDeltaTime)) / 50, 4));

            if (currObj.Angle != null && lastObj.Angle != null)
            {
                double angleDifference = Math.Abs(currObj.Angle.Value - lastObj.Angle.Value);
            ...
            if (currObj.Index > 2)
            {
                double o1 = calculateOverlapFactor(currObj, lastObj);
                double o2 = calculateOverlapFactor(currObj, lastLastObj);
                double o3 = calculateOverlapFactor(lastObj, lastLastObj);
            ...
            if (currObj.Angle != null)
            {
                ...
                flowDifficulty += Math.Sqrt(currVelocity) *
                                  SnapAimEvaluator.CalcAcuteAngleBonus(currObj.Angle.Value) *
            ...
            if (Math.Max(prevVelocity, currVelocity) != 0)
            {
                ...
                double overlapVelocityBuff = Math.Min(OsuDifficultyHitObject.NORMALISED_DIAMETER * 1.25 / Math.Min(currObj.AdjustedDeltaTime, lastObj.AdjustedDeltaTime),
