...
namespace osu.Game.Rulesets.Osu.Difficulty.Evaluators.Aim
{
    public static class SnapAimEvaluator
    {
        ...
        public static double EvaluateDifficultyOf(DifficultyHitObject current, bool withSliderTravelDistance)
        {
            ...
            double acuteAngleBonus = calculateAcuteAngleBonus(osuCurrObj, osuLastObj, currVelocity, prevVelocity, currDistance);
            ...
            snapDifficulty += calculateVelocityChangeBonus(osuCurrObj, osuLastObj, currVelocity, prevVelocity, currDistance, withSliderTravelDistance);
            snapDifficulty += calculateSliderBonus(osuCurrObj, withSliderTravelDistance);
        ...
        private static double calculateAcuteAngleBonus(OsuDifficultyHitObject curr, OsuDifficultyHitObject last, double currVelocity, double prevVelocity, double currDistance)
        {
            ...
            if (Math.Max(curr.AdjustedDeltaTime, last.AdjustedDeltaTime) >= 1.25 * Math.Min(curr.AdjustedDeltaTime, last.AdjustedDeltaTime))
                return 0;
            ...
            double acuteAngleBonus = CalcAngleAcuteness(currAngle);
            ...
            acuteAngleBonus *= velocityInfluence * DiffUtils.Smootherstep(DiffUtils.MillisecondsToBPM(curr.AdjustedDeltaTime, 2), 300, 400) *
                               DiffUtils.Smootherstep(currDistance, 0, diameter * 2);

            return acuteAngleBonus * acute_angle_multiplier;
        }

        private static double calculateWideAngleBonus(OsuDifficultyHitObject curr, OsuDifficultyHitObject last, double currDistance, double prevDistance, bool withSliderTravelDistance)
        {
            if (curr.Angle is not double currAngle || last.Angle is not double lastAngle)
                return 0;
            ...
            double wideAngleCurrVelocity = currDistance / DiffUtils.Pow(curr.AdjustedDeltaTime, wide_angle_time_scale);
            double wideAnglePrevVelocity = prevDistance / DiffUtils.Pow(last.AdjustedDeltaTime, wide_angle_time_scale);

            if (last.BaseObject is Slider && withSliderTravelDistance)
            {
                double sliderDistance = last.LazyTravelDistance + curr.LazyJumpDistance;
                wideAngleCurrVelocity = Math.Max(wideAngleCurrVelocity, sliderDistance / DiffUtils.Pow(curr.AdjustedDeltaTime, wide_angle_time_scale));
            }

            wideAngleBonus *= Math.Min(wideAngleCurrVelocity, wideAnglePrevVelocity);

            var last2 = (OsuDifficultyHitObject)curr.Previous(2);

            if (last2 != null)
            {
                // If objects just go back and forth through a middle point - don't give as much wide bonus
                // Use Previous(2) and Previous(0) because angles calculation is done prevprev-prev-curr, so any object's angle's center point is always the previous object
                var lastBaseObject = (OsuHitObject)last.BaseObject;
                var last2BaseObject = (OsuHitObject)last2.BaseObject;
        ...
        /// </summary>
        private static double calculateWiggleBonus(OsuDifficultyHitObject curr, OsuDifficultyHitObject last, double currVelocity, double prevVelocity, double currDistance, double prevDistance)
        {
            if (curr.Angle is not double currAngle || last.Angle is not double lastAngle)
                return 0;
        ...
        private static double calculateVelocityChangeBonus(OsuDifficultyHitObject curr, OsuDifficultyHitObject last, double currVelocity, double prevVelocity,
                                                           double currDistance, bool withSliderTravelDistance)
        {
            ...
            double overlapVelocityBuff = Math.Min(diameter * 1.25 / Math.Min(curr.AdjustedDeltaTime, last.AdjustedDeltaTime), Math.Abs(prevVelocity - currVelocity));

            double velocityChangeBonus = overlapVelocityBuff * distRatio;

            // Penalize for rhythm changes.
            velocityChangeBonus *= DiffUtils.Pow(Math.Min(curr.AdjustedDeltaTime, last.AdjustedDeltaTime) / Math.Max(curr.AdjustedDeltaTime, last.AdjustedDeltaTime), 2);
        ...
        private static double calculateSliderBonus(OsuDifficultyHitObject curr, bool withSliderTravelDistance)
        {
            ...
            double sliderBonus = curr.TravelDistance / curr.TravelTime;
            return (sliderBonus < 1 ? sliderBonus : DiffUtils.Pow(sliderBonus, 0.75)) * slider_multiplier;
        }
